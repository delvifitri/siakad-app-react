  ...
  if (!payment) {
    return (
      <SimpleLayout title="Pembayaran Tidak Ditemukan">
        <div className="p-4">
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Pembayaran Tidak Ditemukan</h1>
          <p className="mt-4 text-gray-700 dark:text-gray-300">Data pembayaran yang Anda cari tidak ditemukan.</p>
        </div>
      </SimpleLayout>
    );
  }
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{payment.type}</h1>
          ...
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-yellow-700 dark:text-yellow-300">{payment.dueDate}</p>
              </div>
            </div>
          </div>

          <p className="text-gray-700 dark:text-gray-300 text-sm">{payment.description}</p>
        </div>
          ...
          <div className="space-y-4">
            {payment.breakdown.map((item, index) => (
              <div key={index} className="flex justify-between items-center py-2">
                ...
                <span className="font-medium text-gray-900 dark:text-gray-100">Rp {item.amount.toLocaleString()}</span>
              </div>
            ))}
