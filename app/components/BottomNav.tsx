...
export default function BottomNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 w-full bg-white/40 dark:bg-gray-800/40 backdrop-blur-md border-t border-white/10 dark:border-black/10 shadow-lg rounded-t-4xl overflow-hidden"
        ...
        {items.map((it) => {
          ...
          return (
            <li key={it.to} className="flex-1">
              <Link to={it.to} aria-label={it.label} className={`flex flex-col items-center gap-1 py-2 ${active ? "text-blue-600" : "text-gray-700 dark:text-gray-200"}`}>
                <span className={`${active ? "bg-blue-100/70 dark:bg-blue-900/40 text-blue-600" : "bg-white/0 dark:bg-transparent"} p-2 rounded-full`}>{it.icon}</span>
              </Link>
            </li>
          );
        })}
